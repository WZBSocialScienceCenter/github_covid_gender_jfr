...
# YEARS = (2020, 2019)                            # specify the years to collect
YEARS = (2021, 2020)
if len(sys.argv) > 1:
    COLLECTION_DATE = sys.argv[1]
else:
    COLLECTION_DATE = '2020-05-19'
INPUT_USERS_CSV = 'data/usersfull_%s.csv'
#INPUT_USERS_CSV = 'data/concat_users.csv'
#OUTPUT_CSV = 'data/contribs_scraped.csv'
OUTPUT_SUFFIX = '_upd21'
OUTPUT_PICKLE_FMT = 'data/contribs_scraped_%s_%d%s.pickle'
...
CACHE_FILE_RESULTS = 'cache/%s-scrape_contribs_results_new_fmt.pickle' % COLLECTION_DATE
SLEEP_TIME_SEC = 0
...
if '%s' in INPUT_USERS_CSV:
    INPUT_USERS_CSV = INPUT_USERS_CSV % COLLECTION_DATE

print('reading user data from', INPUT_USERS_CSV)
users = pd.read_csv(INPUT_USERS_CSV)
#users = users.head(200)

print(' > loaded %d user accounts' % len(users))

#%%

users = users.loc[~users.gender.isna() &
                  ~users.location_lat.isna() &
                  ~users.location_lng.isna(), ['id', 'login']].reset_index(drop=True)
n_users = len(users)
print(' > %d user accounts left after filtering' % n_users)
    ...
    while i_year < len(YEARS) and not abort_script:
        ...
        if (user_id, year) in contribs_data_per_user:
            ...
            if contribs_resp_ok:
                ...
                days_elems = contribs_soup.select('.js-calendar-graph-svg rect.ContributionCalendar-day')
                ...
                contribs_data = contribs_data.loc[contribs_data.date.str.startswith(str(year)),
                                                  ['user_id', 'date', 'count']]
                print('> got data for %d days (%d contributions)' % (len(contribs_data), contribs_data['count'].sum()))
...
while True:
    if i_added % 10000 == 0 or len(contribs_data_per_user_list) == 0:
        ...
        if partialdfs:
            ...
            full.to_pickle(OUTPUT_PICKLE_FMT % (COLLECTION_DATE, i_files, OUTPUT_SUFFIX))
            partialdfs = []
            i_files += 1
