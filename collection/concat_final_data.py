...
FILTER_CONTRIBS_MAX_DATE = '2021-07-04'    # only retain contribution data up until this date
...
if skip_contribs:
    ...
    picklefiles_all = glob.glob('data/contribs_scraped*.pickle')
    picklefiles_upd = sorted(glob.glob('data/contribs_scraped*_upd21.pickle'))
    picklefiles = sorted(set(picklefiles_all) - set(picklefiles_upd)) + picklefiles_upd   # to get the order right: update last
    assert len(picklefiles) == len(picklefiles_all)
    for i_contribs_pickle, contribs_pickle in enumerate(picklefiles):
        ...
        for aggregtype in AGGREGATION:
            ...
            if aggregtype not in contribs_dfs.keys():
                ...
                contribs_dfs[aggregtype] = pd.concat([contribs_dfs[aggregtype], df_aggreg])\
                    .sort_values(['user_id', 'date', 'count'])\
