...
load_userdata <- function(last_contribs_date, created_before) {
  ...
  userdata <- mutate(userdata,
    ...
    filter(created_at < created_before)
...
load_contribs <- function(aggregtype, from_cache = NULL, remove_week53 = TRUE) {
  ...
  rename(contribs, num_contribs = count)
}
...
prepare_contribs <- function(userdata, contribs, latest_date = NULL, cutoff_quantile = NULL) {
  ...
  contribs <- inner_join(contribs, account_creations, c('user_id' = 'id')) %>%
...
load_prepared_contribtimes <- function(daily_extreme_quantile = NULL) {
  usercontribs <- read.csv('contribtimesdata/contribtimes_individ.csv', stringsAsFactors = FALSE) %>%
    mutate(time = ymd_hms(time),
           date = ymd(date),
           covid_start_date = ymd(covid_start_date))
  
  if (!is.null(daily_extreme_quantile)) {
    daily_contribs_user <- count(usercontribs, date, login)
    thresh <- quantile(daily_contribs_user$n, daily_extreme_quantile)
    print(sprintf('will remove obs. above %f quantile, i.e. with daily contrib. above %d', daily_extreme_quantile, as.integer(thresh)))
    extremes_daily <- filter(daily_contribs_user, n > thresh)
    usercontribs_tmp <- anti_join(usercontribs, extremes_daily, by = c('date', 'login'))
    stopifnot(nrow(usercontribs) - nrow(usercontribs_tmp) == sum(extremes_daily$n))
...
filter_countries <- function(userdata, contribs, min_n_users = 30, only_selected_countries = TRUE, req_single_geocoding_result = TRUE) {
  if (only_selected_countries) {
    print('using only selected countries:')
  ...
  if (req_single_geocoding_result) {
    print('using only results with unambiguous geocoding result')
    nbefore <- nrow(userdata)
    userdata <- filter(userdata, n_geocode_results == 1)
    print(paste('removed', nbefore - nrow(userdata), 'accounts'))
  }
  
  counts_ccode_gender <- count(userdata, country_code, gender) %>% spread(gender, n, fill = 0)
  
  print(paste('using accounts from countries with at least', min_n_users, 'users per country'))
  good_ccodes <- filter(counts_ccode_gender, female >= min_n_users, male >= min_n_users) %>%   # require at least min_n_users of *both* genders
    pull(country_code)
  print(paste('> retaining', paste(good_ccodes, collapse = ', ')))
  print(paste('> removing', paste(setdiff(unique(userdata$country_code), good_ccodes), collapse = ', ')))
  
  nbefore <- nrow(userdata)
  userdata <- filter(userdata, country_code %in% good_ccodes)
  print(paste('removed', nbefore - nrow(userdata), 'accounts'))
  print(paste(nrow(userdata), 'accounts remaining'))
...
aggregate_per_country_and_gender <- function(userdata, contribs_weekly, add_running_month = TRUE, check_max_running_month = TRUE) {
