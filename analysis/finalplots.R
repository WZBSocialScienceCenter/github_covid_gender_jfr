...
sufficient_n_ccodes <- filter(num_accounts_cntry_gender, gender == 'female', n >= 99) %>%
  pull(country_code)
sufficient_n_ccodes
...
countries <- sort(unique(contribs_diff$country_code))
...
num_accounts_cntry_gender <- inner_join(num_accounts_cntry_gender, countrycodes2names, by = 'country_code')

num_accounts_cntry_gender

######################################
## Graph of select countries paper ##
######################################
country_categories <- list(
  'Americas' = c('BR', 'US' ),
  'Eastern Europe/Central Asia' = c('RO', 'RU'),
  'Western Europe' = c('DE', 'IE', 'IT', 'NO'),
  'Asia/Pacific' = c('AU', 'IN', 'CN')
)

selected_countries <- unique(unlist(country_categories))
...
(p_actual_forec <- wrap_plots(actual_forec_plots) +
    plot_layout(ncol = 1, heights = sapply(country_categories, function(cntrs) { length(cntrs) - 0.33 }))) # +

ggsave('articleplots/forecast_vs_actual_select.png', p_actual_forec, width = 8, height = 14)


  
######################################
## Graph for Appendix all countries ##
###################################### 

country_categories <- list(
  'Americas' = c('AR',  'BR', 'CA', 'CO','MX','US' ),
  'Eastern Europe/Central Asia' = c('PL','RO', 'RU', 'TR', 'UA'),
  'Western Europe' = c('AT', 'BE', 'CH', 'DE', 'DK', 'ES', 'FI', 'FR', 'GB', 'IE', 'IT', 'NL', 'NO',  'PT'),
  'East Asia/Pacific' = c('AU', 'CN','ID', 'KR', 'NZ', 'PH', 'TW'),
  'South Asia' = c('BD', 'IN', 'PK'),
  'Middle East/Africa' = c('EG', 'ZA' )   
)
...
for (cntry_cat in names(country_categories)) {
  ...
  p <- ggplot(cat_plt_data, aes(x = date, y = value)) +
    geom_line(aes(color = name)) +
    geom_vline(aes(xintercept = covid_start_date), linetype = 'dashed') +
    geom_text(data = nlabels, aes(x = nlabels_date, y = y, label = label), hjust = 0.5, vjust = 1, size = 2) +
    scale_color_manual(values = actual_forec_pal,
                       limits = c('pred', 'actual'),
                       labels = c('pred' = 'predicted', 'actual' = 'actual'),
                       name = '',
...
(p_actual_forec <- wrap_plots(actual_forec_plots) +
  ...
  # ))
ggsave('articleplots/forecast_vs_actual.png', p_actual_forec, width = 8, height = 23)
...
cntry_gender_trends_categ_summarized <- arrange(cntry_gender_trends_categ, icept_categ_female, slope_categ_female, icept_categ_male, slope_categ_male, country_code) %>%
...
cntry_gender_trends_categ_summarized %>%
  print(n=100)

cntry_gender_trends_categ_summarized_ints <- mutate(cntry_gender_trends_categ_summarized, across(-country, ~ifelse(.x == 'up', 1L, ifelse(.x == 'down', -1L, 0L)))) %>%
  mutate(sum_fem = icept_fem + slope_fem,
         sum_male = icept_male + slope_male)
cntry_gender_trends_categ_summarized_ints
table(cntry_gender_trends_categ_summarized_ints$sum_fem)
table(cntry_gender_trends_categ_summarized_ints$sum_male)

sum(cntry_gender_trends_categ_summarized$icept_fem == 'up')
...
kable(cntry_gender_trends_tabledata,
  ...
  add_footnote("Summary of the overall deviation patterns by gender and countries based on the results from country-specific OLS regressions; arrows in the \"intercept\" column indicate whether men's and women's productivity initially increased ($\\uparrow$), decreased ($\\downarrow$), or stayed the same ($\\rightarrow$) after the first lockdown measures were implemented; arrows in the \"slope\" column indicate the time trend over the observation time.", escape = FALSE) %>%
...
ggsave(sprintf('articleplots/forecastdiff_%s.png', smooth_method),
       p_forecastdiff, width = 8, height = 23)
