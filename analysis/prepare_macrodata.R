...
sosurvey_cn <- filter(sosurvey_full, country_code == 'CN') %>% select(year, gender, hobby)

group_by(sosurvey_cn, year) %>%
  summarise(share_hobby = mean(hobby), n = n())

group_by(sosurvey_cn, year, gender) %>%
  summarise(share_hobby = mean(hobby), n = n()) %>%
  pivot_wider(names_from = gender, values_from = c(share_hobby, n))
