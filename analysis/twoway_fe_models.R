...
sufficient_n_ccodes <- filter(num_accounts_cntry_gender, gender == 'female', n >= 99) %>%   # ZA has 99
  pull(country_code)
sufficient_n_ccodes

data <- read.csv("aggregdata/finaldata_unbalanced.csv", stringsAsFactors = FALSE) %>%
  filter(country_code %in% sufficient_n_ccodes) %>%
  rename(country_char  = country_code) %>% 
  mutate(country_char = as.factor(country_char),
         gender = as.factor(gender)) %>%
  mutate(country_code  = as.integer(country_char),
...
count(data, gender, week)

max_week <- count(data, gender, week) %>%
  group_by(gender) %>%
  summarise(max_week = max(week)) %>%
  ungroup() %>%
  summarise(max_week =  min(max_week)) %>%
  pull(max_week)

data <- filter(data, week <= max_week)
...
margins <- lapply(c("closedschools", "curfews", "workplace"), function(var) {
  ...
  lapply(unique(data$female), function(g) {
    ...
    subsetdata <- filter(data, female == g) %>%
      select(country_code, week_fct, closedschools, curfews, workplace, diff)
    print('> fitting model')

    m <- lm.cluster(substitute(diff ~ closedschools + curfews + workplace + closedschools:workplace +
                                  i*factor(country_code) + week_fct,
                              list(i = as.name(var))),
...
cntrys_li <- c("BR", "US", "RO", "RU", "AU", "IN", "CN", 'DE', 'IE', 'IT', 'NO')
...
coefplot.data <- right_join(margins, cntrys_df, by = 'country_code') %>% 
  ...
  mutate(variable = factor(variable, ordered = T,
         ...
         country_gr = as.factor(case_when(country_char %in% c("BR", "US") ~ "Americas",
                                country_char %in% c("RO", "RU") ~ "Eastern Europe /\nCentral Asia",
                                country_char %in% c('AU', 'IN', 'CN') ~ 'Asia/Pacific',
                                TRUE ~ "Western Europe")))
...
#### models M0 to M3 (table 4 in paper) ####
...
m0_intercepts <- lapply(0:1, function(female) {
  ...
  m <- lm.cluster(diff ~ factor(country_code) + week_fct,
                  cluster = 'country_code',
                  data = subsetdata)
...
m1_icept_strngcy <- lapply(0:1, function(female) {
  subsetdata <- data[data$female == female,]
  m <- lm.cluster(diff ~ covid_stringency + factor(country_code) + week_fct,
                  cluster = 'country_code',
                  data = subsetdata)
...
m2_icept_policies <- lapply(0:1, function(female) {
  subsetdata <- data[data$female == female,]
  m <- lm.cluster(diff ~ closedschools + curfews + workplace + factor(country_code) + week_fct,
                  cluster = 'country_code',
                  data = subsetdata)
...
m3_icept_policies_int <- lapply(0:1, function(female) {
  subsetdata <- data[data$female == female,]
  m <- lm.cluster(diff ~ closedschools + curfews + workplace + closedschools:workplace + factor(country_code) + week_fct,
                  cluster = 'country_code',
                  data = subsetdata)
  coi_df <- coeffs_of_interest(m, c('(Intercept)', 'closedschools', 'curfews', 'workplace', 'closedschools:workplace'))
  coi_df$female <- female
  select(coi_df, female, everything())
}) %>%
  bind_rows() %>%
  mutate(model = 'M3') %>% 
  select(model, female, everything())
m3_icept_policies_int


bind_rows(m0_intercepts, m1_icept_strngcy, m2_icept_policies, m3_icept_policies_int)

# data_filtered <- mutate(data, policies_sum = closedschools + workplace + curfews) %>%
#   filter(policies_sum < 2)
# 
# count(data_filtered, country_char)
# filter(data_filtered, closedschools == 1) %>% nrow()
# filter(data_filtered, workplace == 1) %>% nrow()
# filter(data_filtered, curfews == 1) %>% nrow()
# 
# cor(data_filtered[c('closedschools', 'workplace', 'curfews')])
# 
# 
# m2_icept_policies2 <- lapply(0:1, function(g) {
#   subsetdata <- filter(data_filtered, female == g) %>%
#     select(country_code, week, closedschools, curfews, workplace, diff)
#   m <- lm.cluster(diff ~ closedschools + curfews + workplace + factor(country_code),
#                   cluster = 'country_code',
#                   data = subsetdata)
#   coi_df <- coeffs_of_interest(m, c('(Intercept)', 'curfews', 'closedschools', 'workplace'))
#   coi_df$female <- g
#   select(coi_df, female, everything())
# }) %>%
#   bind_rows() %>%
...
# 
# 
# m <- lm.cluster(diff ~ female + closedschools + curfews + workplace +
#                        female:closedschools + female:curfews + female:workplace +
#                        factor(country_code) + female:factor(country_code),
#                 cluster = 'country_code',
#                 data = data_filtered)
# 
# marg <- margins(m$lm_res, vcov = m$vcov, 
#                 variable = c('closedschools', 'curfews', 'workplace'), 
#                 at = list(female = 0:1))
# summary(marg)
