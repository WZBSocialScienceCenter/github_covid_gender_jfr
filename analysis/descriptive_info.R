...
result <- filter_countries(userdata, contribs_weekly, min_n_users = 99)
userdata <- result[[1]]
contribs_weekly <- result[[2]]
...
count(userdata, gender)

#### number of contributions in total ####

sum(contribs_weekly$num_contribs)

#### number of contribution dates in total ####

nrow(contribs_weekly)
...
# overall prop. of weeks without any contributions in 2019
mean(usercontribs_weekly19$num_contribs == 0)

# prop. of weeks without any contributions in 2019 per gender
group_by(usercontribs_weekly19, gender) %>%
  summarise(prop_no_contrib = mean(num_contribs == 0))

# prop. of users who contributed in less than 10% of all weeks
group_by(usercontribs_weekly19, id) %>%
  summarise(num_weeks_no_contrib_user = sum(num_contribs == 0)) %>%
  summarise(mean_prop_few_contrib_user = mean(num_weeks_no_contrib_user > 47))

# mean weekly contrib. by gender 2019
...
#### pre-COVID example timeline for US ####

us_precovid <- filter(contribs_weekly_gender_with_NAs, country_code == 'US', date < '2020-03-01')

(p <- ggplot(us_precovid, aes(x = date, y = mean_contribs)) +
  geom_line(aes(color = gender)) +
  scale_color_brewer(palette = 'Dark2', guide = guide_legend(title = '')) +
  labs(title = 'Pre-COVID activity in the US', x = '', y = 'Mean number of contributions per week'))
ggsave('plots/precovid_timeline_contribs_US.png', p, width = 8, height = 4)
